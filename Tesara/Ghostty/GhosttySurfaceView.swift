...
class GhosttySurfaceView: NSView, NSTextInputClient {
    ...
    @IBAction func paste(_ sender: Any?) {
        ...
        // Text takes priority (existing behavior)
        if let pastedText = pasteboard.string(forType: .string), !pastedText.isEmpty {
        ...
        // Fall back to image: save as temp PNG and paste the file path.
        // No confirmPaste — the path is generated by Tesara, not external input.
        if let imagePath = saveClipboardImage(from: pasteboard) {
            sendText(imagePath)
        }
    }
    ...
    private func saveClipboardImage(from pasteboard: NSPasteboard) -> String? {
