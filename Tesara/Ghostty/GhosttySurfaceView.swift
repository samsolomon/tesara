...
class GhosttySurfaceView: NSView, NSTextInputClient {
    ...
    init(app: ghostty_app_t, config: GhosttySurfaceConfig, initialSize: NSSize? = nil) {
        ...
        registerForDraggedTypes([.string, .fileURL, .URL, .png, .tiff])
    ...
    @IBAction func paste(_ sender: Any?) {
        guard let resolved = resolveContent(from: NSPasteboard.general) else { return }
        if resolved.isPath || confirmPaste(resolved.text) {
    ...
    /// Resolve pasteboard content into terminal-ready text.
    /// Priority: file URLs → text → raw image data (as temp PNG path).
    private func resolveContent(from pasteboard: NSPasteboard) -> (text: String, isPath: Bool)? {
        // 1. File URLs (e.g., files from Finder)
        ...
        // 2. Text (existing behavior)
        if let text = pasteboard.string(forType: .string), !text.isEmpty {
            return (text, false)
        }

        // 3. Raw image data → save temp PNG, return escaped path
        if let path = Self.saveClipboardImage(from: pasteboard) {
            return (Self.shellEscape(path), true)
        }
    ...
    /// Save clipboard image as a temp PNG and return the path, or nil.
    private static func saveClipboardImage(from pasteboard: NSPasteboard) -> String? {
        let url = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("tesara-paste-\(UUID().uuidString).png")

        // Fast path: raw PNG bytes already on the pasteboard — write directly, no decode/encode
        ...
        // Fallback: decode TIFF via ImageIO and write as PNG (avoids NSImage triple-copy)
    ...
    override func draggingEntered(_ sender: any NSDraggingInfo) -> NSDragOperation {
        guard let types = sender.draggingPasteboard.types,
              !Set(types).isDisjoint(with: [.string, .fileURL, .URL, .png, .tiff]) else {
            return []
        }
